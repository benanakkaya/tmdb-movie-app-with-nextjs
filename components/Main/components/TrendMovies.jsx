    ...
    return (
        <div className='px-[20px] grid grid-cols-2 md:grid-cols-4 gap-3 pb-[20px]'>
            ...
            <Link className='relative group flex justify-center' href={`/movie/${movie.id}`}>
                ...
                <strong className='text-white hidden md:block font-bold absolute bottom-6'>{movie.title}</strong>
                <strong className='text-white hidden md:block font-bold absolute bottom-2'>({movie.original_title})</strong>
           </Link>
            ))}
        </div>
    )
